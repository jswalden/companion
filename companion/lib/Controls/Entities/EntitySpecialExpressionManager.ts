...
import type { StoreActionResultTargetValue } from './Types.js'
...
export class EntityPoolSpecialExpressionManager {
	...
	constructor(
		...
		this.#specialExpressions = {
			...
			storeActionResultTarget: {
				entities: new Map(),
				computeNewValueFn: this.#computeStoreActionResultTarget.bind(this),
				updateFn: updateFns.storeActionResultTarget,
				logger: LogController.createLogger(
					`Controls/${controlId}/EntityPoolSpecialExpressionManager/storeActionResultTarget`
				),
			},
		}
	}
	...
	#computeStoreActionResultTarget(
		entity: ControlEntityInstance,
		wrapper: EntityWrapper,
		parser: VariablesAndExpressionParser,
		logger: Logger
	): StoreActionResultTargetValue {
		const storeActionResultTargetFields = entity.rawStoreActionResultTarget
		if (storeActionResultTargetFields === undefined) {
			wrapper.lastReferencedVariableIds = null
			return undefined
		}

		const variableNameResult = evaluateString(storeActionResultTargetFields.variableName, parser)
		...
		if (storeActionResultTargetFields.target === 'local-variable') {
			const locationResult = evaluateString(storeActionResultTargetFields.location, parser)
		...
		return {
			...
			variableName,
			createIfNotExists: storeActionResultTargetFields.createIfNotExists,
		}
	}
